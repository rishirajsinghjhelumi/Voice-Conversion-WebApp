...
if($#ARGV != 2)
{
	print "Usage: perl mcd.pl <actual directory - indir> <synthesized directory - indir> <mcd file - outfile>\n";
	exit;
}
...
foreach my $file(readdir synthdir)
{
	...
	if($file =~ /\.mcep$/)
	{
		...
		my $output;
		my $output = `/home/rocker/BTP/Source Code/VC_Test/MCD/bin/track_diff_mcd $act_file $syn_file | awk '{print \$2}'`;
		...
		else
		{
			...
			my $output = `//home/rocker/BTP/Source Code/VC_Test/MCD/bin/track_diff_mcd $act_file $temp_syn_file | awk '{print \$2}'`;
			#`rm $temp_syn_file`;
